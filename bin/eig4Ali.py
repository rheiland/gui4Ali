...
from mcds_cells import CellsTab
...
# Callback when the dropdown widget with cached results in used
def read_config_cb(_b):
    ...
    # if Path(config_file).is_file() and first_time:
    ...
    if is_dir:
        # svg.create_all_tracks(read_config.value)
        svg.update(read_config.value)
        cells_tab.update(read_config.value)
        sub.update(read_config.value)

        cells_tab.update('')
    else:  # may want to distinguish "DEFAULT" from other saved .xml config files
        # FIXME/TODO: really need a call to clear the visualizations
        # svg.create_all_tracks()  # TODO: really do this?
        svg.update('')
        cells_tab.update('')
        sub.update('')
...
def write_config_file_cb(b):
    path_to_share = os.path.join('~', '.local','share','eig4Ali')
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/eig4Ali")  # does Windows like this?
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "eig4Ali")
        except:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB eig4Ali' -t 'Your Run completed.'&")
    ...
    svg.update(rdir)
    cells_tab.update(rdir)
    sub.update(rdir)

    # if (svg.show_tracks):
#         print('----- run_done_func: create_all_tracks')
#         svg.create_all_tracks()
...
#         # something on NFS causing issues...
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 eig4Ali-r7 config.xml")   # "-r7" suffix??
        else:
...
titles = ['About', 'Config Basics', 'Microenvironment', 'User Params', 'Out: Cells(svg)', 'Out: Cells', 'Out: Substrates', 'Code' ]
tabs = widgets.Tab(children=[about_tab.tab, config_tab.tab, microenv_tab.tab, user_tab.tab, svg.tab, cells_tab.tab, sub.tab, code_tab.tab],
                   _titles={i: t for i, t in enumerate(titles)},
                   layout=tab_layout)

homedir = os.getcwd()

tool_title = widgets.Label(r'\(\textbf{eig4Ali}\)')
if nanoHUB_flag or hublib_flag:
...
cells_tab.update(output_dir)

# Apparently don't need/want (no guarantee of .svg files in /tmpdir?)
# if (svg.show_tracks):
    # print('--- at startup: show_track ON: calling create_all_tracks')
    # svg.create_all_tracks()
